<template>
  <div class="w-full lg:w-1/3 mt-6 lg:mt-0 overflow-hidden space-y-6" v-bind="$attrs">
    <div class="w-full flex items-center justify-between">
      ...
      >
        <span>More</span>
        <Icon size="16" name="mdi:chevron-right" />
      </button>
    </div>
              ...
              <tr v-for="data in datasets" :key="data.id" class="border-b border-gray-200">
                ...
                <td class="py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <Icon v-if="data.increase" size="14" name="mdi:plus-thick" class="text-emerald-500" />
                    <Icon v-else size="14" name="mdi:minus-thick" class="text-red-500" />
                    <span>${{ data.price }}</span>
                  </div>
                </td>
                <td class="hidden sm:block whitespace-nowrap">
                  <div>
                    <LineChart class="w-28 h-12 -mx-2" :data="data.data" :increase="data.increase" />
                  </div>
                </td>
              </tr>
            </tbody>
