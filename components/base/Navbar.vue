    ...
    <div class="flex flex-col max-w-screen-xl px-8 mx-auto lg:items-center lg:justify-between lg:flex-row py-4">
      <div class="flex flex-col lg:flex-row items-center space-x-4 xl:space-x-8">
        <div class="w-full flex flex-row items-center justify-between py-6">
          <div>
            <NuxtImg src="/img/logo/nefa.svg" class="w-24 xl:w-28" alt="Nefa Logo" />
          </div>
          <button class="rounded-lg lg:hidden focus:outline-none focus:shadow-outline" @click="open = !open">
            <Icon v-if="!open" size="24" name="mdi:segment" />
            <Icon v-else size="24" name="mdi:close" />
          </button>
        </div>
        <ul
        ...
        >
          ...
          <li class="relative group">
            ...
            >
              <span>Products</span>
              <Icon v-if="dropdownNavbar" size="16" name="mdi:chevron-up" />
              <Icon v-else size="16" name="mdi:chevron-down" />
            </button>
